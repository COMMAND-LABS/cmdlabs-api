...
#
# disable_existing_loggers=False is NOT optional. fileConfig defaults to True,
# which switches off every logger that already exists when it runs — and when
# alembic is driven in-process (tests/conftest.py builds the test schema with
# command.upgrade) every `src.*` module logger has already been created by the
# app import above it. With the default, those loggers go silent for the rest
# of the process and application logging vanishes with no error to explain it.
