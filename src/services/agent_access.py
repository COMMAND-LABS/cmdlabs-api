...
There was a third arm — belonging to a SPACE the agent had been shared into —
and it was the only one that crossed an org boundary. Spaces were removed to
simplify the platform, so the answer here is now entirely org-confined. If it
comes back, keep it textually separate from the other two the way it was: one
expression to review when asking "how can this leave its tenant?".
"""
...
def can_access_agent(db: Session, account_id: int, agent_id: int,
    ...
    """
    return access.can_access(db, account_id, access.AGENT, agent_id,
                             required="use", org_id=org_id)
...
def get_accessible_agent_ids(db: Session, account_id: int, org_id: int | None = None) -> set:
    ...
    Grants only, and since spaces were removed that is the whole story. Shares
    used to be added by the LIST queries themselves, through
    org_scope.scoped_resources, which is also where the org predicate they had
    to sit beside lived. Adding them here as well would have double-counted them
    into one caller and left another with the arm applied twice at different
    widths — the same reason applies to whatever replaces them.
    """
